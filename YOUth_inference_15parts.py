...
if __name__ == '__main__':
    ...
    for subject in sorted(os.listdir(args.input_folder)):
        output_folder = os.path.join(args.output_folder, subject)
        os.makedirs(output_folder, exist_ok=True)
        for filename in os.listdir(os.path.join(args.input_folder, subject)):
            if filename.endswith(".png") or filename.endswith(".jpg"):
                filepath = os.path.join(args.input_folder, subject, filename)
                output_files = ['%s/%s_%d.png'%(output_folder, filename.split('.')[0], i) for i in range(5)]
                all_exists = True
                ...
                # # Save segmentation maps to a single file
                ...
                # # cur_canvas = cv2.imread(filepath)
                # # canvas = cv2.addWeighted(seg_canvas, 0.6, cur_canvas, 0.4, 0)
                # cv2.imwrite(output_file_png, seg_canvas)
