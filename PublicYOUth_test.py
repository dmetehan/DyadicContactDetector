...
def test_model(model, model_path, data_loader, args):
    model.load_state_dict(torch.load(model_path))
    model.eval()
    model = model.to(device)
    print(f'Testing on {"test set" if args.test_set else "validation set"} using {model_path}')
    y_pred = []
    y_true = []
...
def main():
    ...
    train_loader, validation_loader, test_loader = init_datasets_with_cfg(root_dir, root_dir, cfg)
    test_model(model, model_path, test_loader if args.test_set else validation_loader, args)
