...
def crop(img, bbxes, person_ids):
	...
	x1, y1, x2, y2 = min(p[0]['x1'], p[1]['x1']), min(p[0]['y1'], p[1]['y1']), max(p[0]['x2'], p[1]['x2']), max(p[0]['y2'], p[1]['y2'])
	...
	# actual cropping
	x1, y1, x2, y2 = max(0, int(round(x1)) - dx_left), max(0, int(round(y1)) - dy_top), min(width, int(round(x2)) + dx_right), min(height, int(round(y2)) + dy_bottom)
	...
	for img_name in ann_data:
		...
		for ci in ann_data[img_name]['ci_classif']:
			...
			try:
				...
				print("Unable to write the crop:", crop_path)
			count += 1
