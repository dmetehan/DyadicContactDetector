...
class GraphCNN(nn.Module):
    ...
    def __init__(self, A, num_layers=5, num_channels=512):
        ...
        self.fc = nn.Linear(21, 2, bias=True)
...
def initialize_gcn_model(cfg, device, finetune=False, signatures=False):
    adjmat = get_adj_mat()
    adjmat = torch.Tensor(adjmat).to(device)
    model = GraphCNN(adjmat, num_channels=256, num_layers=5).to(device)
    if signatures:
